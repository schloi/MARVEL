...
static int process_guide(ScaffoldContext* ctx, char* path, int ignore_missing)
{
    ...
    while ( ( line = fgetln( fin, NULL ) ) )
    {
        ...
        if ( seqid == -1 )
        {
            printf("unknown sequence %s in line %" PRIu64 "\n", seq, nline);
            err = 1;
            break;
        }
...
{
    printf("rescaffolding %" PRIu64 " sequences\n", ctx->nsgnodes);
    ...
    do
    {
        ...
        {
            sgnodes[ guide[i] ].flags |= NODE_ACTIVE;
        }

        for ( i = wnd_prev ; i < nguide && i < wnd_prev + wnd_size ; i++ )
        {
            score_node(ctx, guide[i], 1);
        }
...
{
    printf( "%s [-lms n] [-p string] [-o path.prefix] "
            "-L string links.in [... links.in] [-L ...] "
            "-g file.in -f fasta.in -d digest_sites.in\n", app );
...
int main( int argc, char* argv[] )
{
    ...
    int c;
    while ( ( c = getopt( argc, argv, "d:f:g:l:L:m:p:s:o:" ) ) != -1 )
    {
        switch ( c )
        {
            case 'd':
