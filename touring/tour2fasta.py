...
def write_path_corrected(fileOut, seqname_prefix,
    ...
    if bComp:
        ...
        pathRidUncorrected = [ (rid, trim_e, trim_b) ]
    else:
        pathRid = [ (ridc, trim_b_c, trim_e_c) ]
        pathRidUncorrected = [ (rid, trim_b, trim_e) ]
    ...
    for e in path:
        ...
        if bComp:
            ...
            pathRidUncorrected.append( (rid, ovh_end, trim_b) )
            ...
            pathRidUncorrected.append( (rid, ovh_beg, trim_e) )
    ...
    # TODO: make writing of reads, sreads and ureads optional

    fileOut.write(">{}_{} path={} ends={},{} length={} reads={} sreads={} ureads={}\n{}\n".format(
                    seqname_prefix, pathid, pathid,
                    e1, e2,
                    len(pathSeq),
                    ",".join( [ "{},{},{}".format(*x) for x in pathRid ] ),
                    ",".join( [ str(x) for x in pathSource ] ),
                    ",".join( [ "{},{},{}".format(*x) for x in pathRidUncorrected ] ),
                    wrapSeq(pathSeq)))
...
def write_path(fileOut, seqname_prefix, db, track_source, track_trim, vread, eflags, elen, eend, path, pathid, e1, e2):
    ...
    # TODO: make writing of reads and sreads optional
...
def main():
    ...
    parser.add_argument("-p", "--prefix", metavar = "file.prefix", default = None, help = "fasta sequence name prefix")
