...
def vertexEdgeQuality(g, v, eend, ediv):
    nSum = 0
    n = 0
...
def best_next_node(g, vstart, vflags, vread, eflags, eend, next_dir, lookahead):
    ...
    maxv = None
    items = list( sorted( dicCounts.items(), cmp = lambda x,y: cmp(y[1], x[1] ) ) )
...
def backbone(g, vstart, vflags, vread, eflags, eend, ediv, elen, vpath, vcont, bid, circular, lookahead, ilookahead):
    path = []
    ...
    while True:
        ...
        out_edges.sort( lambda x,y: cmp( elen[y], elen[x] ) )
        ...
        for e in out_edges:
            ...
            if vread[v] == 119842:
                print("DEBUG {}->{} {}".format( vread[e[0]], vread[e[1]], eflag))
...
def graph_compute_paths(g, arrStartVertices, bbdist_prop,
                        vflags_prop, vread_prop, vpath_prop, vcont_prop,
    ...
    while True:
        ...
        while vcur < len(arrStartVertices):
            ...
            if flags & ( V_BACKBONE | V_DISCARD | V_DEAD_END | V_MODULE ):
                continue
        ...
        (path, ends) = backbone(g, v,
                        vflags_prop, vread_prop, eflags_prop, eend_prop, ediv_prop, elen_prop, vpath_prop, vcont_prop,
        ...
        if len(path) > 0:
            ...
            logging.info("found path consisting of {} reads".format(len(path) + 1))

            while len(path) > 0 and eflags_prop[ path[0] ] & LAS.OVL_MODULE:
                del path[0]
                start += 1

            while len(path) > 0 and eflags_prop[ path[-1] ] & LAS.OVL_MODULE:
                del path[-1]
                end += 1
...
def process_graph(g, dropinversions, circular, lookahead, ilookahead):
    ...
    vcolor_prop = {}
    vstyle_prop = {}
    bbdist_prop = { key : -1 for key in g.nodes() }
    vpath_prop = { key : -1 for key in g.nodes() }

    vflags_prop = { key : 0 for key in g.nodes() }

    edir_prop = {}
    ewidth_prop = { key : 1.0 for key in g.edges() }

    vsize_prop = { key : 0.2 for key in g.nodes() }

    vquality_prop = { key : 0.0 for key in g.nodes() }

    vread_prop = { n : g.node[n]["read"] for n in g.nodes() }
    vcont_prop = { n : g.node[n]["contained"] for n in g.nodes() }
    ...
    eflags_prop = { e : int(g.edge[e[0]][e[1]]["flags"]) for e in g.edges() }
    ediv_prop = { e : int(g.edge[e[0]][e[1]]["divergence"]) for e in g.edges() }
    elen_prop = { e : int(g.edge[e[0]][e[1]]["length"]) for e in g.edges() }
    ...
    for v in g.nodes():
        fQuality = vertexEdgeQuality(g, v, eend_prop, ediv_prop)
...
def drop_inversions(g, eend, eflags):
    ...
    for v in g.nodes():
        if len( v_all_edges(g, v) ) == 0: # v.in_degree() == 0 and v.out_degree() == 0:
            setDiscard.add(v)
...
def paths_write(fpath, paths):
    filePaths = open(fpath, "w")
...
def main():
    ...
    parser.add_argument("-c", "--circular", help = "allow circular paths", action = "store_true")
    parser.add_argument("-l", "--lookahead", help = "lookahead during touring", default = DEF_PATH_LOOKAHEAD, type = int)
