...
void Set_Filter_Params( int kmer, int binshift, int suppress, int hitmin, int nthread, int traceSpace )
{
    if ( kmer > 32 )
    {
...
static void* mask_thread( void* arg )
{
    ...
    char* s = ( (char*)( TA_block->bases ) ) + TA_block->reads[ beg ].boff;
    ...
    else
    {
        ...
        const uint64 _Cumber[4] = { Cumber[0], Cumber[1], Cumber[2], Cumber[3] };
...
static void* compsize_thread( void* arg )
{
    ...
    const KmerPos * const src    = FR_src;
    int n, i, c, p;
    uint64 h, g;
...
{
    Merge_Arg* data  = (Merge_Arg*)arg;
    const KmerPos* const asort   = MG_alist;
    const KmerPos* const bsort   = MG_blist;
    const DAZZ_READ* const reads = MG_bblock->reads;
