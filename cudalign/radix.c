...
typedef struct
{
    ...
    int LEX_byte;     //  Current byte to sort on
    int LEX_next;     //  Next byte to sort on (if >= 0)
    int64 LEX_zdiv;   //  Size of thread segments (in bytes)
    Element* LEX_src; //  Source data goes to ...
    Element* LEX_trg; //  Target data
...
static void* lex_thread( void* arg )
{
    ...
    Element* src  = data->LEX_src;
    Element* trg  = data->LEX_trg;
    int64 zdiv    = data->LEX_zdiv;
    ...
    if ( data->LEX_next < 0 )
        ...
        {
            x        = tptr[ src[ i ].vector[ data->LEX_byte ] ]++;
            trg[ x ] = src[ i ];
        }
    else
        for ( i = data->beg; i < n; i++ )
        {
            q        = src[ i ].vector;
            x        = tptr[ q[ data->LEX_byte ] ]++;
            trg[ x ] = src[ i ];
            sptr[ ( ( x / zdiv ) << 8 ) | q[ data->LEX_next ] ] += 1;
        }
    return ( NULL );
}
...
static void* lexbeg_thread( void* arg )
{
    ...
    n = data->end;
    for ( i = data->beg; i < n; i++ )
        tptr[ src[ i ].vector[ data->LEX_byte ] ] += 1;
    return ( NULL );
}
...
void* Radix_Sort( int64 len, void* src, void* trg, int* bytes )
{
    ...
    {
        parmx[ i ].LEX_zdiv = ( len - 1 ) / NTHREADS + 1;
        parmx[ i ].LEX_src  = (Element*)src;
        parmx[ i ].LEX_trg  = (Element*)trg;
    }
    ...
    for ( b = 0; bytes[ b ] >= 0; b++ )
    {
        ...
        */
        ...
        for ( i = 0; i < NTHREADS; i++ )
        {
            parmx[ i ].LEX_byte = bytes[ b ];
            parmx[ i ].LEX_next = bytes[ b + 1 ];

            parmx[ i ].beg = x;
            x              = parmx[ i ].LEX_zdiv * ( i + 1 );
            if ( x > len )
                x = len;
            parmx[ i ].end = x;
            for ( j = 0; j < 256; j++ )
        ...
        {
            xch                = parmx[ i ].LEX_src;
            parmx[ i ].LEX_src = parmx[ i ].LEX_trg;
            parmx[ i ].LEX_trg = xch;
        }
